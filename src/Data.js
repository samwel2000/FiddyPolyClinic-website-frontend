import Sliderimg1 from './assets/images/background2.jpg';
import Sliderimg2 from './assets/images/background3.jpg';
import Consultation from './assets/images/consultation.webp';
import Emergency from './assets/images/emergency.webp';
import Pharmacy from './assets/images/pharmacy.webp';
import Laboratory from './assets/images/laboratory.webp';
import Radiology from './assets/images/radiology.webp';
import Medical from './assets/images/medical.webp';
import Member from './assets/images/team_member.webp';
import Doctor from './assets/images/Doctor.webp';


export const phoneNumber = "+255 652 068 080"

export const Email = "info@fiddypolyclinic.co.tz"

export const SliderContent = [
    {
        'id':1,
        'title':'We provide health care solutions',
        'image': `${Sliderimg1}`,
        'description':'a clinic where both general and specialist examinations and treatments are available to outpatients.'
    },
    {
        'id':2,
        'title':'We provide top medical services',
        'image':`${Sliderimg2}`,
        'description':'a clinic where both general and specialist examinations and treatments are available to outpatients.'
    },
    {
        'id':3,
        'title':'Medical services that you can trust',
        'image':`${Sliderimg1}`,
        'description':'a clinic where both general and specialist examinations and treatments are available to outpatients.'
    }
]

export const ServiceContent1 = [
    {
        'id':1,
        'heading':'Medical councelling',
        'icon': `${Medical}`,
        'description':'Phasellus at nunc orci. Donec ipsum metus, pharetra quis nunc sit amet, maximus vehicula nibh. Praesent pulvinar porta elit, a commodo erat accumsan sed. Vivamus vel tristique nibh.'
    },
    {
        'id':2,
        'heading':'Qualified doctors',
        'icon': `${Consultation}`,
        'description':'Phasellus at nunc orci. Donec ipsum metus, pharetra quis nunc sit amet, maximus vehicula nibh. Praesent pulvinar porta elit, a commodo erat accumsan sed. Vivamus vel tristique nibh.'
    },
    {
        'id':3,
        'heading':'Emergency services',
        'icon': `${Emergency}`,
        'description':'Phasellus at nunc orci. Donec ipsum metus, pharetra quis nunc sit amet, maximus vehicula nibh. Praesent pulvinar porta elit, a commodo erat accumsan sed. Vivamus vel tristique nibh.'
    }
]

export const ServiceContent2 = [
    {
        'id':1,
        'heading':'Pharmacy',
        'icon': `${Pharmacy}`,
        'description':'Phasellus at nunc orci. Donec ipsum metus, pharetra quis nunc sit amet, maximus vehicula nibh. Praesent pulvinar porta elit, a commodo erat accumsan sed. Vivamus vel tristique nibh.'
    },
    {
        'id':2,
        'heading':'Laboratory',
        'icon': `${Laboratory}`,
        'description':'Phasellus at nunc orci. Donec ipsum metus, pharetra quis nunc sit amet, maximus vehicula nibh. Praesent pulvinar porta elit, a commodo erat accumsan sed. Vivamus vel tristique nibh.'
    },
    {
        'id':3,
        'heading':'Radiology',
        'icon': `${Radiology}`,
        'description':'Phasellus at nunc orci. Donec ipsum metus, pharetra quis nunc sit amet, maximus vehicula nibh. Praesent pulvinar porta elit, a commodo erat accumsan sed. Vivamus vel tristique nibh.'
    }
]

export const TeamMembers = [
    {
        'id':1,
        'name':'Dr. Hafidh Ameir',
        'image': `${Doctor}`,
        'title':'Health consultant'
    },
    {
        'id':2,
        'name':'Samwel Godfrey',
        'image':`${Member}`,
        'title':'Health consultant'
    },
    {
        'id':3,
        'name':'Yusuph Ameir',
        'image':`${Doctor}`,
        'title':'Statistician'
    },
    {
        'id':4,
        'name':'Dr. Hafidh Ameir',
        'image': `${Member}`,
        'title':'Health consultant'
    },
    {
        'id':5,
        'name':'Samwel Godfrey',
        'image':`${Doctor}`,
        'title':'Health consultant'
    },
    {
        'id':6,
        'name':'Yusuph Ameir',
        'image':`${Member}`,
        'title':'Statistician'
    },
    {
        'id':7,
        'name':'Dr. Hafidh Ameir',
        'image': `${Member}`,
        'title':'Health consultant'
    },
    {
        'id':8,
        'name':'Samwel Godfrey',
        'image':`${Doctor}`,
        'title':'Health consultant'
    },
    {
        'id':9,
        'name':'Yusuph Ameir',
        'image':`${Member}`,
        'title':'Statistician'
    }
]
